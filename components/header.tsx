...
export function Header() {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-50 border-b border-[var(--border)] bg-[var(--background)]/80 backdrop-blur-md">
        ...
        <nav className="hidden items-center gap-8 md:flex" aria-label="주요 메뉴">
          {nav.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="text-sm text-[var(--muted)] transition hover:text-violet-600 dark:hover:text-violet-400"
            >
              {item.label}
            </a>
          ))}
      ...
      <div
        id="mobile-menu"
        className={`border-t border-[var(--border)] bg-[var(--background)] md:hidden ${open ? "block" : "hidden"}`}
      >
        <nav className="flex flex-col gap-1 px-4 py-3" aria-label="모바일 메뉴">
          {nav.map((item) => (
            <a
              key={item.href}
              href={item.href}
              className="rounded-lg px-3 py-2.5 text-sm text-[var(--foreground)] transition hover:bg-[var(--accent-soft)]"
